...
for(const [file,v] of Object.entries(files)){fs.mkdirSync(path.dirname(file),{recursive:true});fs.writeFileSync(file,v.encoding==='base64'?Buffer.from(v.data,'base64'):v.data);}
console.log(`V13 source restored from GitHub bundle: ${Object.keys(files).length} files.`);
