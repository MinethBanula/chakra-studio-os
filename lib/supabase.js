...
function rowsOf(input){return Array.isArray(input)?input:[input]}

async function normalizePayload(table,input){
  const schemas=await loadColumns()
  ...
  get(target, prop, receiver){
    ...
    return table=>{
      ...
      return new Proxy(builder,{
        get(b,p,r){
          if(p==='insert') return async (payload,options)=>b.insert(await normalizePayload(table,payload),options)
          if(p==='update') return async (payload,options)=>b.update(await normalizePayload(table,payload),options)
          if(p==='upsert') return async (payload,options)=>b.upsert(await normalizePayload(table,payload),options)
